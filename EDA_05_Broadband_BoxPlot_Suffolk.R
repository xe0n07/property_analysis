...
cat("\nDistinct Suffolk postcode-district groups (used as 'town') : ", nrow(town_counts))
...
box_plot = ggplot(plot_data, aes(x = town_code, y = avg_speed)) +
  ...
  coord_flip() +
  labs(
    title = "Average Download Speed by Town - Suffolk",
    subtitle = "Grouped by postcode district (e.g. IP4, IP28). Minimum 20 postcodes per group.",
